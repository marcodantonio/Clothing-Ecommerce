...
<html lang="en">
      ...
      $totcosto=$_SESSION["totalecosto"];
    if(isset($_COOKIE['idindirizzo'])){  
    $indirizzo=$_COOKIE['idindirizzo']; } //id indirizzo  0 -> nuovo immesso (dovrebbe avere max +1 della tabella ma non va la query) || id effettivo di una carta
      $spedizione=$_COOKIE['idspedizione'];  //== 'standard' or =='express'
      
  //id carta di credito  'alla_consegna' || 0 -> nuovo immesso (dovrebbe avere max+1 anche questa) || id effettivo di una carta
        $id=$_SESSION['id'];
       ...
       $citta= $_SESSION['indirizzo_citta'];
        ...
        if(isset($_SESSION['carta_proprietario'])){
        $proprietario= $_SESSION['carta_proprietario'];
        $numero =$_SESSION['carta_numero'];
        $data_scadenza= $_SESSION['carta_data'];
        $cvv =$_SESSION['carta_cvv'];


        insert_cart($connect,$id, $proprietario, $numero, $data_scadenza, $cvv);}
    
       if(isset($_COOKIE['idconsegna'])){  
            $carta=$_COOKIE['idconsegna']; 
           if($carta=="alla_consegna"){
               $idpagamento=2;
    ...
    $query="SELECT *   FROM indirizzo where id=$indirizzo";
    $dati=mysqli_query($connect, $query);
    $valori=mysqli_fetch_array($dati);
    $fullname=$valori['email'];
    $email=$valori['email'];
    $via=$valori['via'];
    $cap=$valori['cap'];
    $citta=$valori['citta'];
    ...
    $query="insert into ordine VALUES ('', '$codicefinale', '$stato_pagamento', '$spedizione', 'In corso', '$data', '1', '$idpagamento','$indirizzo', '$fullname','$email', '$via', '$cap', '$stato', '$citta') ";
    $dati=mysqli_query($connect, $query);
     ...
     $result=carrello_accesso($connect,$_SESSION['id']);
                ...
                if($sconto) {
                    ...
                    $costo=$prezzoscontato;}
                        ...
                        $query="insert into dettaglio_prodotto VALUES ('','$codice', '$nome_prodotto', '$costo','$foto', '$categoria', '$brand', '$colore', '$taglia', '$quantita')";
                        $dati=mysqli_query($connect, $query);
                        //echo("error: " . mysqli_error($connect));
                        
                            $query="SELECT MAX(id) as massimo FROM dettaglio_prodotto";
                            $dati=mysqli_query($connect, $query);
                            $valori=mysqli_fetch_array($dati);
                            $iddettaglioprodotto=$valori["massimo"];
                        
                          $query="insert into prodotto_ordine VALUES ('$iddettaglioprodotto', '$idordine')";
                           $dati=mysqli_query($connect, $query);
                            ...
                            $nuovaquantita=$vecchiaQuantità-$quantita;
                        
                            $query="UPDATE taglia_prodotto set quantita='$nuovaquantita' where(id_taglia=$idtaglia and id_prodotto=$idP)";
                            $dati=mysqli_query($connect, $query);
   ...
   // setcookie("cond_indirizzo", "0", time()-1);
  ...
  <body>
      ...
      <nav class="navbar navbar-expand-lg bg-transparent border-0 shadow-0 navbar-light px-lg-5 ">
        <div class="collapse navbar-collapse" id="navbarContent">
          ...
          <ul class="list-inline mb-0 d-none d-lg-block">
              ...
              <li class="list-inline-item mr-3"><a class="text-dark text-hover-primary position-relative" href="wishlist.php">
                      <svg class="svg-icon navbar-icon">
        ...
        <div class="hero-content">
          ...
          <div>
              <div role="alert" class="d-flex align-items-center alert alert-success">
                  <svg class="svg-icon w-3rem h-3rem svg-icon-light flex-shrink-0 mr-3">
                      <use xlink:href="#checked-circle-1"> </use>
                  </svg>Your order is confirmed</div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="pb-6">
      <div class="container">
        <p class="lead">Thank you, <?php echo $_SESSION["nome"] . "&nbsp;" . $_SESSION["cognome"]; ?>. Your order is confirmed.</p>
        <p class="lead mb-5">Your order hasn't shipped yet but we will send you ane email when it does. </p>
    <?php echo"    <p class='mb-6'><a class='btn btn-outline-dark' href='customer-order.php?id=$idordine'>View or manage your order</a></p>"; ?>
        <div class="p-5 bg-gray-100">
          <div class="row">
              <?php
              $result=select_ordine_effettuato($connect, $idordine);
              $valori = mysqli_fetch_array($result);
              $data=$valori["data"];
              $idcorriere=$valori["id_corriere"];
              $query="SELECT * FROM corriere WHERE id=$idcorriere";
              $dati=mysqli_query($connect, $query);
              $valoriC = mysqli_fetch_array($dati);
              $corriere=$valoriC["nome_corriere"];
              
              
              ?>
            <div class="col-6 col-lg-3 mb-5 mb-lg-0">
              <div class="text-sm text-uppercase text-muted mb-3">Order no.</div><span class="h5"><?php echo "#$codicefinale"; ?></span>
            </div>
            <div class="col-6 col-lg-3 mb-5 mb-lg-0">
              <div class="text-sm text-uppercase text-muted mb-3">Date</div><span class="h5"><?php echo $data; ?></span>
            </div>
            <div class="col-6 col-lg-3">
              <div class="text-sm text-uppercase text-muted mb-3">Total</div><span class="h5"><?php echo "$$totcosto"; ?>              </span>
            </div>
            <div class="col-6 col-lg-3">
              <div class="text-sm text-uppercase text-muted mb-3">Shipping</div><span class="h5"><?php echo "$corriere"; ?>           </span>
            </div>
          </div>
        </div>
      </div>
